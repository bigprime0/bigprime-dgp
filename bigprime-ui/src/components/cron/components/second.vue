...
const radioChange = () => {
  switch (radioValue.value) {
    ...
    case 2:
      ...
      cron.value.second = cycle01.value + '-' + cycle02.value
      break
    case 3:
      average01.value = checkNum(average01.value, 0, 58)
      average02.value = checkNum(average02.value, 1, 59 - average01.value || 0)
      cron.value.second = average01.value + '/' + average02.value
      break
    case 4:
      cron.value.second = isEmpty(checkboxList.value) ? '*' : checkboxList.value.join()
      break
  }
}
