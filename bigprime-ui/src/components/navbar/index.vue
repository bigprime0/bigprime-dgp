  ...
  <div class="navbar">
    <div class="left-side">
      <div style="display: flex; align-items: center">
        <img style="max-height: 36px" src="/bigprime.png" @click="jumpUrl" />
        <h5 @click="jumpUrl">{{$t('top.main.text')}}</h5>
      </div>
    </div>
    <ul class="right-side">
      <li>
      ...
      <li class="navbar-user">
        <tiny-user-head
          v-if="!isEmpty(userStore.userInfo.avatarUrl)"
        ...
        <tiny-user-head
          v-else
          v-model="userStore.userInfo.realName"
          class="head-item"
        ...
        <div class="trigger-user">
          ...
          >
            ...
            <iconCloudUpload v-if="item.value === 3"></iconCloudUpload>
            <iconCheckOut v-if="item.value === 4"></iconCheckOut>
            <span v-if="item.value !== 3">{{ $t(item.label) }}</span>
            <tiny-file-upload
              v-if="item.value === 3"
...
import { FileStorageService } from '@/services/currency/file-storage'
...
const iconCloudUpload = IconCloudUpload()
const lan = ref(false)
...
const userBox = [
  ...
  { label: 'messageBox.changeAvatar', value: 3 },
...
const fileParam = ref({ attachmentUuid: userStore.userInfo.avatar })
const uploadFile = (file: any) => {
  FileStorageService.uploadAvatar(file).then((result) => {
    userStore.userInfo.avatarUrl = result.domainUrl
    if (isEmpty(userStore.userInfo.avatar)) {
      userStore.userInfo.avatar = result.attachmentUuid
    }
  })
}
</script>
