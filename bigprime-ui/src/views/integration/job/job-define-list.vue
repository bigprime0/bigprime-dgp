<template>
  <div :class="queryModel.type === 0 ? 'container-list' : ''">
    <Breadcrumb
      v-if="queryModel.type === 0"
      :items="['menu.integration', 'menu.integration.define']"
    />
    <div :class="queryModel.type === 0 ? 'contain' : ''" v-loading="isLoading">
      ...
      >
          ...
          <div class="grid-toolbar">
            ...
            <div>
              <tiny-button
                v-if="queryModel.type === 0"
                type="primary"
                @click="handleAddSeatunnelJob"
              >
                {{ $t('integration.job.add.seatunnel') }}
              </tiny-button>
              <tiny-button
                v-if="!(queryModel.type === 1 && isEmpty(queryModel.writeSourceTable))"
                type="primary"
                @click="handleAddAddaxJob"
              >
        ...
        <tiny-grid-column
          field="engine"
          :title="$t('integration.job.column.engine')"
          show-overflow
        ></tiny-grid-column>
        <tiny-grid-column
          field="jobName"
          :title="$t('integration.job.column.jobName')"
          show-overflow
        ></tiny-grid-column>
        <tiny-grid-column
          v-if="queryModel.type === 1"
          field="writeSourceTable"
          :title="$t('integration.job.column.tableName')"
          show-overflow
        ></tiny-grid-column>
        <tiny-grid-column
          field="creator"
      ...
      >
        <div>
          <JobSeatunnel
            :key="nanoid(8)"
            v-if="selectProduct == 'seatunnel'"
            :job="job"
            :selectConfigs="selectConfigs"
            :envFormData="envFormData"
            :sourceFormData="sourceFormData"
            :transformFormData="transformFormData"
            :sinkFormData="sinkFormData"
          ></JobSeatunnel>
          <JobAddax
            :key="nanoid(8)"
            v-if="selectProduct == 'addax'"
            :job="job"
            :selectConfigs="selectConfigs"
            :envFormData="envFormData"
            :sourceFormData="sourceFormData"
...
const handleRowSubmit = (row: integrationJob) => {
  //isLoading.value = true
  IntegrationConfigService.submitJob(row)
  Modal.message({
    message: t('addax.submit.message'),
    status: 'success'
  })
