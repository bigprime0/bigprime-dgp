<template>
  <div class="container-list">
    <div class="contain">
      ...
      <keep-alive>
        <JobDefineForm
          v-if="stepValue == 'setting'"
          :step="stepValue"
          :needFilter="needFilter"
          :subcategories="subcategories"
          :stepConfigs="stepConfigs"
          :JobDefineFormData="envFormData"
      ...
      <keep-alive>
        <JobDefineForm
          v-if="stepValue == 'reader'"
          :step="stepValue"
          :needFilter="needFilter"
          :subcategories="subcategories"
          :stepConfigs="stepConfigs"
          :JobDefineFormData="sourceFormData"
          :source-data="sourceData"
          :job="job"
        ></JobDefineForm>
      </keep-alive>
      <keep-alive>
        <JobDefineForm
          v-if="stepValue == 'writer'"
          :step="stepValue"
          :needFilter="needFilter"
          :subcategories="subcategories"
          :stepConfigs="stepConfigs"
          :JobDefineFormData="sinkFormData"
          :source-data="sourceData"
          :job="job"
          :form-disabled="queryModel.type === 1"
        ></JobDefineForm>
      </keep-alive>
      <keep-alive>
        <monacoEditor
          ref="buildRef"
          v-if="stepValue == 'build'"
...
const _setStepConfig = () => {
  ...
  if (stepValue.value) {
    stepConfigs.value = selectConfigs.value.filter(
      (item) => item.category == stepValue.value || item.subCategory == stepValue.value
    )
    subcategories.value = []
...
const _buildJob = () => {
  ...
  if (sourceFormData.value.plugin_name) {
    JobContext.job.content.reader = _buildPart(sourceFormData, 'reader')
  }

  if (sinkFormData.value.plugin_name) {
    JobContext.job.content.writer = _buildPart(sinkFormData, 'writer')
  }
  ...
  if (JobContext.job.content.writer) {
    ...
    if (JobContext.job.content.writer?.parameter?.connection) {
      writerTables = JobContext.job.content.writer?.parameter?.connection?.table
    }
    if (writerTables && writerTables.length > 0) {
  ...
  let writeSourceDatabase = !isEmpty(job.value.writeSourceDatabase)
    ? `${job.value.writeSourceDatabase}.`
    : ''
  let readerSourceDatabase = !isEmpty(job.value.readerSourceDatabase)
    ? `${job.value.readerSourceDatabase}.`
    : ''
  if (querySql !== '') {
    ...
    readerTables.forEach((table: any) => {
      writerTables.forEach((writerTables: any) => {
        sql.push(
          `INSERT INTO ${writeSourceDatabase}${writerTables} SELECT ${column} FROM ${readerSourceDatabase}${table}`
        )
      })
    })
  }
...
const _buildPart = (formData: any, category: string) => {
  ...
  if (formData.value.plugin_name) {
    ...
    let partConfigs = selectConfigs.value.filter(
      (t) => t.category == category && t.subCategory == formData.value.plugin_name
    )
      ...
      if (find) {
        if (find.isVisible && find.isProperty && find.isEnable && value != null && value != '') {
          if (find.compType == 'array') {
            if (value.indexOf(';') != -1) {
              value = value.split(';')
            } else if (find.configKey != 'querySql' && value.indexOf(',') != -1) {
              value = value.split(',')
...
const handleSave = async () => {
  ...
  if (result) {
    Modal.message({
      message:
        job.value.id > 0 ? t('common.prompt.update.success') : t('common.prompt.save.success'),
      status: 'success'
    })
...
watchEffect(async () => {
  if (queryModel.value.type === 1 && queryModel.value.writeSourceTable) {
    let writer = await IntegrationConfigService.getMasterDataWriter({
      table: queryModel.value.writeSourceTable
    })
