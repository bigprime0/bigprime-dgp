  ...
  <div class="demo-tag-create">
    <span v-for="(tag, index) in dynamicTags" :key="'tiny-tag-' + tag + index">
      <tiny-tooltip v-if="tagIndex !== index" effect="dark" :content="tag" placement="top" :disabled="tag.length < 20">
        <tiny-tag type="success" closable @close="handleClose(index)" @dblclick="handleEdit(tag, index)">
    ...
    <tiny-button type="success" plain v-show="!inputVisible" class="button-new-tag" size="small" @click="showInput">{{$t('assets.asset.field.tag.add')}}</tiny-button>
  </div>
</template>
...
const dynamicTags = defineModel('tags')
