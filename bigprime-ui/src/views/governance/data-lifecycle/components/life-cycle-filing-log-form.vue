      ...
      <tiny-row>
        <tiny-col :span="6">
          <tiny-form-item :class="{'span-change':includes(colorChanges,'tableName')}"
            ...
            <tiny-input v-model="logFormData.tableName"></tiny-input>
          </tiny-form-item>
        </tiny-col>
      </tiny-row>
...
<!--            <tiny-radio v-model="logFormData.status" :label=1>{{ $t('common.operations.status.underReview') }}</tiny-radio>-->
      ...
      <tiny-row>
        <tiny-col :span="6">
          <tiny-form-item :class="{'span-change':includes(colorChanges,'sourceId')}"
            ...
            <tiny-select v-model="logFormData.sourceId" disabled>
              <tiny-option v-for="item in source" :key="item.id" :label="item.name"
                           :value="item.id">
              </tiny-option>
            </tiny-select>
          </tiny-form-item>
        ...
        <tiny-col :span="6">
          <tiny-form-item :class="{'span-change':includes(colorChanges,'warehouseLocation')}"
                          :label="$t('lifecycle.filing.field.warehouseLocation')">
            <tiny-input v-model="logFormData.warehouseLocation" disabled></tiny-input>
          </tiny-form-item>
        </tiny-col>
      </tiny-row>
      ...
      <tiny-row>
        <tiny-col :span="6">
          <tiny-form-item :class="{'span-change':includes(colorChanges,'size')}"
                          :label="$t('lifecycle.filing.field.size')">
            <tiny-input v-model="logFormData.size" disabled></tiny-input>
          </tiny-form-item>
        </tiny-col>
        <tiny-col :span="6">
          <tiny-form-item :class="{'span-change':includes(colorChanges,'rows')}"
                          :label="$t('lifecycle.filing.field.rows')">
            <tiny-input v-model="logFormData.rows" disabled></tiny-input>
          </tiny-form-item>
        </tiny-col>
      </tiny-row>
      ...
      <tiny-row>
        <tiny-col :span="12">
          <tiny-form-item :class="{'span-change':includes(colorChanges,'condition')}"
            ...
            <tiny-input type="textarea" v-model="logFormData.condition" :rows=3 show-word-limit></tiny-input>
          </tiny-form-item>
        </tiny-col>
      </tiny-row>
    </tiny-form>
